...
export default async function AdminCoursesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  function pageHref(p: number) {
    ...
    return (query ? `/admin/courses?${query}` : '/admin/courses') as `/admin/courses`;
  }
    ...
    <div className={styles.page}>
      ...
      {courses.length === 0 ? (
        <div className={styles.empty}>
          ...
          <Link href="/admin/courses" className={styles.emptyLink}>
            Limpiar filtros
          </Link>
        </div>
      ) : (
        <Table>
          ...
          <Table.Body>
            {courses.map((c) => (
              <Table.Row key={c.id}>
                <Table.Cell>
                  <Link href={`/admin/courses/${c.id}`} className={styles.titleCell}>
